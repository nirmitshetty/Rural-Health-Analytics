...
data = pd.read_csv("kanoe_project\pop_covered_statewise\HP_pop.csv")
...
ax.set_title('Average rural population covered in Himachal Pradesh across years(2005-14)')
ax.set_xticks([p + 1.5 * width for p in pos])
ax.set_xticklabels(data['year'])
plt.xlim(min(pos)-width, max(pos)+width*4)
plt.ylim([0, max(data['sub-centre'] + data['phc'] + data['chc'])] )
plt.legend(['sub-centre', 'phc', 'chc'], loc='center')
plt.grid()
