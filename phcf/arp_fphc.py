...
import numpy as np
data = pd.read_csv("C:\Users\Nirmit\kanoe_project\phcf\ARP_fphc.csv")#enter path here
def autolabel(rects):
    for rect in rects:
...
ax.set_title('PHC facilities in Arunachal Pradesh across years')
ax.set_xticks([p + 1.5 * width for p in pos])
ax.set_xticklabels(data['year'])
plt.xlim(min(pos)-width, max(pos)+width*4)
plt.ylim([0, 100] )
plt.legend(['With Labour Room', 'With Operation Theatre', 'With 4-6 Beds','With 24 Hrs. Delivery Facility'], loc='upper left')
plt.grid()
plt.show()