...
import numpy as np
data = pd.read_csv("C:\Users\Nirmit\kanoe_project\o&g_statewise\HP_og.csv")#enter path here
def autolabel(rects):
    for rect in rects:
...
autolabel(rect1)
rect2 = plt.bar([p+width for p in pos],data['P'],width,alpha = 0.5,color='#FFC222',label=data['Year'][1])
autolabel(rect2)
...
ax.set_title('Obstetricians & Gynaecologists in Himachal Pradesh across years')
ax.set_xticks([p + 1.5 * width for p in pos])
ax.set_xticklabels(data['Year'])
plt.xlim(min(pos)-width, max(pos)+width*4)
plt.ylim([0, max(data['R'] + data['P'])] )
plt.legend(['Required',  'InPosition'], loc='upper left')
plt.grid()
plt.show()