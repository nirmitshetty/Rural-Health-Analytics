...
import numpy as np
data = pd.read_csv("C:\Users\Nirmit\kanoe_project\phcf\HP_fphc.csv")#enter path here
def autolabel(rects):
    for rect in rects:
...
ax.set_title('PHC facilities in Himachal Pradesh across years')
ax.set_xticks([p + 1.5 * width for p in pos])
ax.set_xticklabels(data['year'])
plt.xlim(min(pos)-width, max(pos)+width*6)
plt.ylim([0, 100] )
plt.legend([' Without Electric Supply', 'Without Regular Water Supply', 'Without AllWeather Motorable Approach Road','With Telephone','With Computer'], loc='center')
plt.grid()
plt.show()